...
import { generateADHDSentences, generateADHDWords } from '../services/aiService.js';
...
// @desc    Generate 10 simple sentences from a document for ADHD game
router.get('/adhd-sentences/:id', protect, async (req, res) => {
  try {
    const doc = await Document.findById(req.params.id);
    if (!doc) return res.status(404).json({ message: 'Document not found' });

    const sentences = await generateADHDSentences(doc.content, doc.standard || 3);
    res.json({ sentences, title: doc.title, documentId: doc._id });
  } catch (error) {
    console.error('ADHD sentence route error:', error);
    res.status(500).json({ message: error.message || 'Error generating sentences' });
  }
});
...
// @desc    Generate 8 key words from a document for ADHD word game
router.get('/adhd-words/:id', protect, async (req, res) => {
  try {
    const doc = await Document.findById(req.params.id);
    if (!doc) return res.status(404).json({ message: 'Document not found' });

    const words = await generateADHDWords(doc.content);
    res.json({ words, title: doc.title });
  } catch (error) {

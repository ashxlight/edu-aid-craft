...
export const simplifyContent = async (text, disability, grade, language = 'en-IN') => {
  ...
  The output content MUST be written ENTIRELY in ${langName} (translate if necessary). Do NOT output English unless asked to.`;
  ...
  - Be thorough and detailed—write around 300-350 words of adapted content, including an introductory overview, 2 detailed sub-sections, and a solid conclusion.
  - CRITICAL LANGUAGE RULE: ALL output text (simplifiedContent, audioScript, and videoSegments text, slides bullet points) MUST BE written in ${langName}. Only "imageQuery" should be in English.
  ...
  {
    "simplifiedContent": "(STRING) The structured adapted text in ${langName}. MUST be a SINGLE long string. Use ## for section headings. Use numbered lists. Include a **Summary:** paragraph at the end. Write around 400-500 words, explaining the core concepts clearly.",
    "audioScript": "(STRING) A warm, conversational narration (in ${langName}, around 150-200 words). MUST be a SINGLE string containing natural flowing speech. Do NOT use any << >> markers or markdown.",
    "videoSegments": [
      {
        "text": "Intriguing first segment of the explanation (approx 30-40 words, in ${langName}).",
        "imageQuery": "Atmospheric 4k illustration of the beginning of the process"
      },
      {
        "text": "The middle part detailing the transformation or core concept (in ${langName}).",
        "imageQuery": "Detailed 4k scientific diagram or cinematic art"
      },
      {
        "text": "Conclusion and wrap-up with a final takeaway (in ${langName}).",
        "imageQuery": "Calm, summative high-quality educational art"
      }
    ],
    "slides": [
      {
        "title": "Slide Title in ${langName}",
        "bulletPoints": ["Detailed key point 1 with full explanation in ${langName}", "Point 2", "Point 3", "Point 4"],
        "colorTag": "blue",
        "visualIcon": "🌱",
  ...
  VIDEO RULES:
  - Generate exactly 3 video segments. 
  - Each part narration: 50-60 words. High clarity, descriptive.
...
export const generateADHDSentences = async (text, grade = 3) => {
  const prompt = `You are an expert special education teacher helping children with ADHD.
  
Your task: Read the following educational content and generate exactly 4 simple, short sentences that a Grade ${grade} child can read aloud.

Rules:
- Each sentence MUST be taken directly from the core idea of the content (not made up).
- Each sentence MUST be 3-7 words only. Extremely short and clear.
- Use simple, everyday English words. No complex vocabulary.
- Each sentence should be a complete thought (subject + verb).
- Do NOT use questions. Use declarative sentences only.
- Sentences should cover DIFFERENT key points of the text.

Content:
${text.substring(0, 3000)}

Return ONLY a valid JSON object like this, nothing else:
{
  "sentences": [
    "The sun is very hot.",
    "It gives us yellow light.",
    "Plants need the warm sun.",
    "Sunlight helps trees grow big."
  ]
}`;
  ...
  try {
    ...
    console.error('ADHD sentence generation error:', error);
    throw new Error('Failed to generate ADHD sentences');
  }
};

export const generateADHDWords = async (text) => {
  const prompt = `Extract 8 key learning words from the text below for an ADHD child's vocabulary game.
  
Rules:
- Selection: Only important concepts and nouns.
- Length: Max 10 characters per word.
- Simplicity: Avoid complex jargon unless it's a core topic keyword.

Content:
${text.substring(0, 3000)}

Return ONLY JSON:
{ "words": ["Mars", "Sun", "Orbit", "Star", "Space", "Rock", "Sky", "Dust"] }`;

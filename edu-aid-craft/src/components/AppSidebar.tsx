...
const navItems = [
  ...
  { title: "ADHD Racing Adventure", url: "/adhd-game", icon: Gamepad2 },
    ...
    <Sidebar collapsible="icon" className="border-r border-sidebar-border bg-popover">
      <SidebarContent>
        ...
        <SidebarGroup>
          <SidebarGroupContent>
            <SidebarMenu>
              {navItems.filter(it => (!it.teacherOnly || user?.role === "teacher") && (!(it as any).studentOnly || user?.role === "student")).map((item) => (
